...
import build.Main;
import components.core.gamechess.BoardChess;
    ...
    public List<Coordinate> getPossibleMove(Piece[][] board) {
        ...
        for(int i = 0; i < listCoordinate.size(); i++){
            for(int j = 0; j < list.size(); j++){
                ...
                if(listCoordinate.get(i).getRow() == list.get(j).getRow() && listCoordinate.get(i).getCol() == list.get(j).getCol()){  
        ...
        for(int iRow = 0; iRow < 8; iRow++){
            for(int iCol = 0; iCol < 8; iCol++){
                if(BoardChess.chessBoard[iRow][iCol].color !=
                        ...
                        !BoardChess.chessBoard[iRow][iCol].name.equals("WK")&&
                        !BoardChess.chessBoard[iRow][iCol].name.equals("BK")){
        ...
        for(int iRow = 0; iRow < 8; iRow++){
            for(int iCol = 0; iCol < 8; iCol++){                               
                if(board[iRow][iCol].color  != this.color && board[iRow][iCol].color != -1 && 
                        board[iRow][iCol].getName().equals("BP") == false &&
                         board[iRow][iCol].getName().equals("WP") == false &&
                        board[iRow][iCol].getName().equals("WK") == false &&
                        board[iRow][iCol].getName().equals("BK") == false) {
    ...
    public List<Coordinate> checkKingKillByKing(Piece[][] board){
        ...
        try{
            for(int iRow = 0; iRow < 8; iRow++){
                for(int iCol = 0; iCol < 8; iCol++){
                    if(this.color == Piece.Black){
                        if(board[iRow][iCol].name.equals("WK")){
                            listCoordinate = new
                    ...
                    if(this.color == Piece.White){
                        if(board[iRow][iCol].name.equals("BK")){
                            listCoordinate = new
                                    ArrayList<Coordinate>(getCoordinateEmptyOrContainEnemy(board,
                                            (King)board[iRow][iCol]));
                        }
