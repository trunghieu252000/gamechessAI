...
public class BoardChess implements  Serializable{
            ...
            for(int iCol = 1; iCol <= 8; iCol++){
                ...
                if(Main.Mode == Main.PLAYER_ONLINE && Main.capacity == Main.TO_BE_CHALLENGED){
                    if(BoardChess.chessBoardReverse[iRow -1][iCol - 1].getColor() != -1){
                        ImageIcon icon = new ImageIcon("art/pieceArt/" +
                    ...
                    if(BoardChess.chessBoard[iRow -1][iCol - 1].getColor() != -1){
                        ImageIcon icon =  new  ImageIcon("art/pieceArt/" +  BoardChess.chessBoard[iRow -1][iCol - 1].getName()+".png");
    ...
    public static boolean checkMate(){
        ...
        for(int iRow = 0; iRow < 8; iRow++){
            for(int iCol = 0; iCol < 8; iCol++){
                ...
                if(BoardChess.turn == Piece.Black){
                    if(BoardChess.chessBoard[iRow][iCol].getName().equals("BK")){
                        king = new Coordinate(iRow, iCol);
                    }
                }
                if(BoardChess.turn == Piece.White){
                    if(BoardChess.chessBoard[iRow][iCol].getName().equals("WK")){
                        king = new Coordinate(iRow, iCol);
                    }
                }
            }
    ...
    public static boolean breakSecret(){
        List<ChessAndCoordinate> listChessAndCoordiante = new ArrayList<ChessAndCoordinate>();
        ...
        for(int i = 0; i < listChessAndCoordiante.size(); i++){
			...
			for (int j = 0; j < listChessAndCoordiante.get(i).getListCoordinate().size(); j++) {
				int iRowAfter = listChessAndCoordiante.get(i).getListCoordinate().get(j).getRow();
				int iColAfter = listChessAndCoordiante.get(i).getListCoordinate().get(j).getCol();
				...
				BoardChess.chessBoard[iRowAfter][iColAfter] = BoardChess.chessBoard[iRowBefore][iColBefore];
                ...
                //TEST check Mate
				if (checkMate() == false) {
					BoardChess.chessBoard[iRowBefore][iColBefore] = BoardChess.chessBoard[iRowAfter][iColAfter];
					BoardChess.chessBoard[iRowBefore][iColBefore].getCoordinate().setRow(iRowBefore);
					BoardChess.chessBoard[iRowBefore][iColBefore].getCoordinate().setCol(iColBefore);

					BoardChess.chessBoard[iRowAfter][iColAfter] = temp;
					BoardChess.chessBoard[iRowAfter][iColAfter].getCoordinate().setRow(temp.getCoordinate().getRow());
					BoardChess.chessBoard[iRowAfter][iColAfter].getCoordinate().setCol(temp.getCoordinate().getCol());
    ...
    public static List<ChessAndCoordinate> getAllPossibleOfMyChess(){
        List<ChessAndCoordinate> list = new ArrayList<ChessAndCoordinate>();
         // get All chess of Player 
        for(int iRow = 0; iRow < 8; iRow++){
            for(int iCol = 0; iCol < 8; iCol++){
                if(BoardChess.chessBoard[iRow][iCol].getColor() == BoardChess.turn){
                    Coordinate myChess = new Coordinate(iRow, iCol);
                    ChessAndCoordinate temp = new
                            ChessAndCoordinate(BoardChess.chessBoard[iRow][iCol].
